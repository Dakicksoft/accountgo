    ...
    public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
    {
      ...
      try
      {
        if (ModelState.IsValid)
        {
          ...
          var responseString = await responseAddNewUser.Content.ReadAsStringAsync().ConfigureAwait(false);

          JObject resultAddNewUser = JObject.Parse(responseString);

          HttpResponseMessage responseInitialized = null;
          JObject resultInitialized = null;
          if ((bool)resultAddNewUser["succeeded"])
          {
            responseInitialized = Get("administration/initializedcompany");

            responseString = await responseInitialized.Content.ReadAsStringAsync().ConfigureAwait(false);

            resultInitialized = JObject.Parse(responseString);
